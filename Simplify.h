...
namespace Simplify
{
	...
	//
	// target_count  : target no of triangles
	...
	void simplify_mesh(int target_count, double agressiveness=7)
	{
		...
		loopi(0,triangles.size()) triangles[i].deleted=0;	// initialising
		...
		// ready
		//int timeEnd=timeGetTime();					// timer not working
		printf("%s - %d/%d %d%% removed\n",__FUNCTION__,
	...
	void write_obj(char* filename)
	{
		...
		loopi(0,vertices.size())		// write vertices in the file
		{
			fprintf(file, "v %lf %lf %lf\n", vertices[i].p.x,vertices[i].p.y,vertices[i].p.z);
		}	
		loopi(0,triangles.size()) if(!triangles[i].deleted)		//write faces in the file
		{
